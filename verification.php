...
if(isset($_POST['verify'])){
    ...
    if(empty($vcode)){
        ...
        if(mysqli_num_rows($check)==1){
            $statusupdate=mysqli_query($db, "UPDATE `users` SET `status` =1 WHERE `OTP`='$vcode'  AND `email` ='$email'");
            session_unset();
            session_destroy();
            unset($email);
            header("location:login.php?vsuccess");
        }
        else{
...
<form   action="verification.php" method="POST">
    <div  class="text-center p-5 ">
            ...
            <div>
                <a href="login.php" style="font-size:18px;">change Email</a>
            </div>
